...
my $Bin="/share/fg3/lintao/solab/lintao/tomato/06_snp_analysis/03_pca_EIG/EIG5.0.1/bin";
...
close(PARFILE);
$command = "$Bin/smartpca";          # MUST put bin directory in path
$command .= " -p $parfile >$l";
print("$command\n");
system("$command");
...
### cax ploteig
$command = "perl $Bin/ploteig";           # MUST put bin directory in path
$command .= " -i $evec";
$command .= " -c 1:2 ";
$command .= " -p $popstring ";
$command .= " -x ";
...
if ( $i =~ m/\.ped$/ || $i =~ m/\.PED/ )  {
  $command = "perl $Bin/evec2pca-ped.perl $k $evec $b $o";
}
else  {
  $command = "perl $Bin/evec2pca.perl $k $evec $b $o";
}
print("$command\n");
system("$command");
